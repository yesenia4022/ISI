...
if pID == 1
    ...
    switch P.s_profile
        ...
        case 'pulse'
            ...
            temp = temp*P.contrast/100;
        case 'checker'
            sdom2 = x_ecc*cos(P.ori*pi/180-pi/2) - y_ecc*sin(P.ori*pi/180+pi/2); 
...
elseif pID == 2  %set grating variables (plaid)
    ...
    amp = cos(tdom - P.t_phase2*pi/180);  %set to cos in case i'm only using the first value of tdom for inf = t_period
    switch P.t_profile2
