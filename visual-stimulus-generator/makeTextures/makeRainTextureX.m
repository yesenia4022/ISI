...
[xmesh ymesh orimesh bwmesh colormesh] = ndgrid(xdom,ydom,1:length(oridom),1:length(bwdom),1:length(colordom));
...
for k = 1:P.Ndrops
    ...
    xseq = xmesh(seqdum(1:N_Im));  %The sequence for each drop
    yseq = ymesh(seqdum(1:N_Im));  %The sequence for each drop
    oriseq = orimesh(seqdum(1:N_Im));  %I don't know why, but this one should be an index, not the actual ori
    bwseq = bwmesh(seqdum(1:N_Im));  %The sequence for each drop
    colorseq = colormesh(seqdum(1:N_Im));  %The sequence for each drop
