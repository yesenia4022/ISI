...
global Mstate screenPTR screenNum imcolortrx
...
xN = Dim_dest(2);  %stimulus width in pixels
yN = Dim_dest(1);  %stimulus height in pixels

xpos = P.x_pos;
ypos = P.y_pos;

%Get upper and lower limits of the patch in screen pixel coordinates
xlim = [xpos - xN/2    xpos + xN/2]; 
ylim = [ypos - yN/2    ypos + yN/2];
x_ecc1 = single(linspace(xlim(1),xlim(2),xN));  %pixel domain on the screen
y_ecc1 = single(linspace(ylim(1),ylim(2),yN));

for i = 1:length(imcolortrx)
   
    [x_ecc{i} y_ecc{i}] = meshgrid(x_ecc1,y_ecc1);  %pix
    
    x_eccdum = x_ecc{i}; y_eccdum = y_ecc{i}; oneMat = ones(size(y_ecc));
    x_ecc{i} = imcolortrx{i}(1,1)*x_eccdum + imcolortrx{i}(2,1)*y_eccdum + imcolortrx{i}(3,1)*y_eccdum.*x_eccdum + imcolortrx{i}(4,1)*oneMat;
    y_ecc{i} = imcolortrx{i}(1,2)*x_eccdum + imcolortrx{i}(2,2)*y_eccdum + imcolortrx{i}(3,2)*y_eccdum.*x_eccdum + imcolortrx{i}(4,2)*oneMat;
    
    x_ecc{i} = x_ecc{i}-xpos;
    y_ecc{i} = y_ecc{i}-ypos;
end
%%
...
for oriid = 1:length(oridom)
    sdom = x_ecc*cos(oridom(oriid)*pi/180) - y_ecc*sin(oridom(oriid)*pi/180);    %deg
    sdom = abs(sdom);
    Im(find(sdom<barWpix/2)) = 1;
