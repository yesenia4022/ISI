...
for i = 1:Nparam
    ...
    nc = nc*length(paramV);
    if i == 1
        istring = ['1:' num2str(length(paramV))]; %input string for 'ndgrid'
        ostring = ['d{' num2str(i) '}'];  %output string for ndgrid
    else
...
%ndgrid outputs 2D grid, even for 1D input...
% if Nparam == 1
%     d{1} = d{1}(1,:);
% end
...
for rep = 1:nr
    ...
    if get(GUIhandles.looper.randomflag,'value')
        ...
        idhold = 1;  %Condition number that does not get shuffled
        if ~isnan(idhold)
            id = find(seq{rep} == idhold);
            %seq{rep}(idhold) = id;
            seq{rep}(id) = seq{rep}(idhold);
            seq{rep}(idhold) = idhold;
        end
