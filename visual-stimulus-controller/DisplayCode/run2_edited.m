...
%             display('done logging data')
            ...
            t = timer;
            t.StartDelay = ACQ.total_time+10;
            t.TimerFcn = @(~,~)disp('done logging data');
            start(t)
            wait(t)
...
%             end
%             tcourseSum = tcourseSum + tcourse;
%             
%             figure(99),
%             clf
%             tdom = (0:length(tcourse)-1)/ACQ.FPS;
%             subplot(1,2,1), plot(tdom,tcourse(1:end))
%             hold on
%             plot(tdom,tcourseSum/trialno,'r'), xlabel('sec')
%             legend('trial','average')
%             subplot(1,2,2), imagesc(squeeze(mean(squeeze(Tens(:,:,2:end-1)),3))), axis image
%             hold off
%         end
