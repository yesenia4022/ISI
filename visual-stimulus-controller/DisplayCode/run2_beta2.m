...
%         end
        ...
        try
            ...
            %This is inserted for the adaptation experiment.  It makes sure
            %that the stimulus finishes on the really long adapting trial.
            %This needs to happen first to make sure doesn't sit forever.            
                    
            %%%%%%OPTION A%%%%%%%%
%            waitforDisplayResp(1) %shutter will be open during adapter
%             wait(ACQ.vid,ACQ.total_time+2,'logging')
            %%%%%%%%%%%%%%%%%%%%%%%
                     
                       
%             %%%%OPTION B%%%%%%
%Its a bad idea to ask it to wait for display after its done logging data.
%It may never leave the while loop. But we assume the adapter takes a lot
%longer, so its ok.
            wait(ACQ.vid,ACQ.total_time+2,'logging')
            Atrials = 1;  %vector of trials with adapter
%             Atrials = [2:2:16];
            if ~isempty(find(c == Atrials)) %If its an adapter trial,
                if ACQ.MechbtwTrialShutter
                    writePosition(ACQ.servo,ACQ.servoCloseVal) %close shutter
                end
                toc
