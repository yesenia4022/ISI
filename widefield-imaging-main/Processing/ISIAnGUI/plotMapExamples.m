function plotMapExamples(hh)

global f0m_var f0m Tens Tens_var Analyzer symbolInfo idExamp G_handles ACQinfo
...
if ~isempty(idExamp)
    ...
    tcsfTens = zeros(size(f0m{1},1), size(f0m{1},2), length(logdom));
    tcsfTensSE = zeros(size(f0m{1},1), size(f0m{1},2), length(logdom));
    for k = 1:length(logdom)

        id = find(logcond == logdom(k));
        ...
        tcsfTens(:,:,k) = imdum;
        tcsfTensSE(:,:,k) = imdumSE;
    end
    
    for i = 1:length(idExamp(:,1))
        tcsf{i} = squeeze(tcsfTens(idExamp(i,2),idExamp(i,1),:));
        tcsfSE{i} = squeeze(tcsfTensSE(idExamp(i,2),idExamp(i,1),:));
        
        subplot(length(idExamp(:,1)),3,3*i-2)
        
        errorbar(log2(logdom),tcsf{i},tcsfSE{i},'k')
        %plot(log2(logdom),tcsf{i},'.-k')
        set(gca,'XTick',log2(logdom(1:2:end)))
        set(gca,'XTickLabel',round(logdom(1:2:end)*10)/10)
        xlim([log2(logdom(1)-.1) log(logdom(end))+1.5])
