function binarymap = GprocessBinary(f0,hh,hnorm,varargin)

global bsflag Analyzer symbolInfo
...
if ~isempty(hh)  
    ...
    normer = ifft2(abs(fft2(hh)).*fft2(normer));
end

normer(find(normer(:)<0)) = NaN;

%Normalize after smoothing to make division more stable (should be doing high pass after)
%binarymap = binarymap./normer;
%binarymap = binarymap/2; % [-1 1]

%binarymap(find((normer)<=.01)) = NaN;
