...
%%%%
dt = trimmean(diff(Analyzer.syncInfo{1}.acqSyncs),50);
Fs = 1/dt;  %frames/sec
%T = mean(diff(Analyzer.syncInfo{1}.dispSyncs(2:end-1)));  %Period of stimulus based on syncs
try
    T = getParamVal('t_period',0)/frate;
catch
    T = getparam('block_Nimage')*getparam('h_per')/60*2;
end
