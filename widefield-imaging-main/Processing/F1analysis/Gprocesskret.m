...
%Or, just do this simple thing and assume delay is short
%kmap_hor = angle(exp(1i*ang0)+exp(-1i*ang2));
%kmap_vert = angle(exp(1i*ang1)+exp(-1i*ang3));
% % 
id = find(delay_vert > pi/2);
kmap_vert(id) = angle(exp(1i*(kmap_vert(id)+pi)));
id = find(delay_hor > pi/2);
kmap_hor(id) = angle(exp(1i*(kmap_hor(id)+pi)));
