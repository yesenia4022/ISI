function getRFfromRandPos_mouse
...
if Monkbit
    ...
    kernOrisigMa = 15;
    kernXsigMa = 2;
    delayWin = [200 800];  %assume the peak response is within this time window
end
%%%%
...
for c = 1:length(DM.colordom)
    ...
    for p = 1:Ncell
        ...
        imdum = kernplot_b+kernplot_w;
        imstart = imdum(:,:,1);
        impk = imdum(:,:,tau);
        stdstart = std(imstart(:));
        stdpk = std(impk(:));
        TC.SNR{c}(p) = stdpk/stdstart;
        ...
        for q = 1:3
            ...
            switch q
                ...
                case 1   %only plot the on+off
                    ma = max([max(RF) -min(RF(:))]);
                    imagesc(xdom,ydom,RF,[-ma ma]), %title(num2str(param(5)))
...
for p = 1:Ncell
    ...
    subplot(ceil(sqrt(Ncell)),ceil(sqrt(Ncell)),p)
    dum = TC.RFW{1}{p} - TC.RFB{1}{p};
    %dum = TC.RFB{1}{p};
    ma = max(dum(:)); mi = min(dum(:));
    ma = max([-mi ma]);
...
if ~isempty(idExamp)
    for c = 1:length(DM.colordom)
        ...
        for i = 1:Nex
            figure
            subplot(3,2,1)
            ma = max(  [fitB_examp{c}{i}(:); fitW_examp{c}{i}(:)]   );
            imagesc(xdom,ydom,-RFB_examp{c}{i}, [-ma ma])
            axis image
            subplot(3,2,2)
            imagesc(xdom,ydom,-fitB_examp{c}{i}, [-ma ma])
            axis image
            
            subplot(3,2,3)
            imagesc(xdom,ydom,RFW_examp{c}{i},  [-ma ma]   )
            axis image
            subplot(3,2,4)
            imagesc(xdom,ydom,fitW_examp{c}{i}, [-ma ma]   )
            axis image
            
            
            subplot(3,2,5)
            dum = RFW_examp{c}{i} - RFB_examp{c}{i};
            ma = max(dum(:)); mi = min(dum(:));
            imagesc(xdom,ydom,dum,[-ma ma])
            axis image
            subplot(3,2,6)
            [param ffit varaccount] = Gabfit(dum);
            ma = max(ffit(:)); mi = min(ffit(:));
            imagesc(xdom,ydom,ffit,[-ma ma])
            axis image
...
for p = 1:Ncell
    ...
    [param ffit varaccount envfit] = Gabfit(dum);

    if varaccount > 0.7 & TC.SNR{c}(p) > 2
        xpos = param(2)*degperpix;
        ypos = param(1)*degperpix;
        xsize = param(3)*degperpix*2; %sinwave is along this dimension
        ysize = param(4)*degperpix*2;
        sfreq = param(7)/degperpix;  %cyc/deg
        opref = param(5);
        ...
        %envfit = envfit/max(envfit(:));
        %contour(envfit,[.5 .5],'k')
       % hold on
    else
        xpos = NaN;
        ypos = NaN;
    ...
    TC.OMag{c}(p) = ysize;  %kind of like orientation selectivity.
    TC.OSig{c}(p) = 1;
end
