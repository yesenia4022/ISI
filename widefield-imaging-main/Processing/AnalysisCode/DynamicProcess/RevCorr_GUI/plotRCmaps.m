...
%     mapmag = mapmag/max(mapmag);
    ...
    figure, subplot(1,2,1), image(xdom,ydom,phi(IMtens)), title('F1/F0'), axis image 
    plotEXcirc(idExamp,xdom,ydom)
    F1F0dom = {0 ,.5 ,1 ,1.5, 2};
    ...
    subplot(1,2,2), image(xdom,ydom,phi(IMtens)), title('spatial phase'), axis image 
    plotEXcirc(idExamp,xdom,ydom)
    phasedom = {-180 ,-90 ,0 ,90, 180};
    iddom = linspace(1,64,length(phasedom));
    colorbar('YTick',iddom,'YTickLabel',phasedom)
    
    figure,scatter(log2(TC.sfpref{c}),TC.F1F0{c},'.k'), xlabel('sfpref'),ylabel('F1/F0')
