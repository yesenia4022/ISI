function kern = revCorrLS3(cellS,trialdom,hh)
...
%     yhat_all{p} = [];
%     y_all{p} = [];
%     %trialidid = [1 3 6 7 8 13 14];
%     %trialidid = [18 15 2];
%     trialidid = [1:10];
%     for trialid = 1:length(trialidid)
%         
%         T = trialdom(trialidid(trialid));
%         [cond rep] = getcondrep(T);
%         
%         hper = gethper(cond);         
%         Tupdate = Tf*hper;   
...
%     
% %     figure,
% %     scatter(yhat_all{p},y_all{p},'.k')
% %     xlabel('predicted response'), ylabel('actual response')
% % 
% %     [param] = NL(yhat_all{p}(:),y_all{p}(:),[1 muyhat(1) 1 muy(1)]);
% %     domu = unique(yhat_all{p});
...
%Now compute the kernel with autoregression after inverting NL


tauP = round(240/acqPeriod)+1;
...
%     end
    ...
    plotkerns(kern{p},oridom,sfdom,taudom,acqPeriod,alpha)
...
function plotkerns(kern,oridom,sfdom,taudom,acqPeriod,alpha)

kernplot = fliplr(squeeze(mean(kern(:,1:4,:),2)));  %Mean over sfreq... Get ori/time kernel
...
%kernplot = fliplr(squeeze(mean(kern(2:6,:,:),1)));  %Get sf/time kernel
