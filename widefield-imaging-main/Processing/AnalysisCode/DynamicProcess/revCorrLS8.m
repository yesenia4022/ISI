function kern = revCorrLS8(cellS,trialdom,hh)

%3 corrects for the nonlinearity w/ feedforward model, and then computes
%the ARMA model

%8 is a modification of 5 which uses Matlab's system ID functions
...
for p = 7:7
    ...
    for trialid = 1:length(trialdom)    
        ...
        HH = zeros(length(y),length(oridom)*length(sfdom));
        pid = 1;
...
%         y = y(id(1):id(end));
    ...
    nc = 0;
    na = ARord;
    nb = Mord*ones(1,length(HHall(1,:)));
    nk = 0*ones(1,length(HHall(1,:)));
%     orders = [na nb nc nk];
...
%     m = armax(data,orders);
    ...
    for i = 1:length(kerndum(1,:))  %loop through each time point
        dum = kerndum(:,i);
        kern{p}(:,:,length(taudom)-i+1) = reshape(dum,length(sfdom),length(oridom))';  %first dimension will be sf
        blnkResp(i) = mean(kern{p}(end,:,length(taudom)-i+1),2); %Mean over orientation at highest sfreq        
