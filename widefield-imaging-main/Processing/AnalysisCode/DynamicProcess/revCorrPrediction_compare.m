function kern = revCorrPrediction_compare(cellMatActual,cellMatControl,trialdom)
...
if ~ARflag
    tauP = round(1000/acqPeriod)+1;
else
    tauP = round(180/acqPeriod)+1;
end
%tauP = 40;
...
for p = 1:length(idExamp)
    ...
    for trialid = 1:length(trialdom)
        ...
        y = squeeze(cellMatActual{cond}(pID,:,rep));     
...
%% Now Prediction of control case
...
for ploop = 1:length(pdom)
    ...
    y = squeeze(cellMatControl{cond}(pID,:,rep));
