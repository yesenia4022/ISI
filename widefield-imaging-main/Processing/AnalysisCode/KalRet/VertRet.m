...
VertRet_Contour=figure('Name','Vertical Retinotopy- Contour','NumberTitle','off');
    ...
    if ContinueTag == 1
        ...
        VertRetfilename=strcat(AnalDir,ExptID,'_VertRet.eps')
        h = figure(VertRet);
        print (h, '-depsc', VertRetfilename)
        ...
        VertRetContourfilename=strcat(AnalDir,ExptID,'_VertRet_Contour.eps')
        h = figure(VertRet_Contour);
        print (h, '-depsc', VertRetContourfilename)
    end
end

% FigureHandles = [VertRet, VertRet_Contour];
% 
