function DataS = AnalyzeColorSFmatrix(imstate,E_yx,varargin)
...
imMat = cell(length(sfdom),length(cdom));
for cond = 1:length(f0m)-1  
    
    theta = getVal('theta',cond);
    cid = find(cdom == theta);
    
    t_period = getVal('s_freq',cond);
    tid = find(sfdom == t_period);
...
for t = 1:length(sfdom)
    for c = 1:length(cdom)
        colorF0(:,:,c) = colorF0(:,:,c)+imMat{t,c}/length(sfdom);
    end
end
for c = 1:length(cdom)
...
%% Get a single sf tuning curve for each color, and plot

id = find(imstate.areaBounds);

tempAll = cell(1,length(cdom));
for c = 1:length(cdom)
    tempAll{c} = zeros(size(imMat{1,1},1),size(imMat{1,1},2),length(sfdom));
    for t = 1:length(sfdom)
         dum = imMat{t,c};
         dum = ifft2(abs(fft2(hh)).*fft2(dum));
         tempAll{c}(:,:,t) = dum;
         
         sftc(c,t) = mean(dum(id));
...
for c = 1:length(cdom)
   sftc(c,:) =  sftc(c,:)/max(sftc(c,:));
end

%Use below for experiments ran with theta [0 45 90 135]
% figure,
% subplot(2,1,1), semilogx(sfdom,sftc([1 3],:)','-o')
% xlabel('s freq')
% set(gca,'XTick',sort(sfdom))
% legend(cleg([1 3])) 
% subplot(2,1,2), semilogx(sfdom,sftc([2 4],:)','-o')
% xlabel('s freq')
% set(gca,'XTick',sort(sfdom))
% legend(cleg([2 4])) 

%Use below for experiments ran with theta [0 90]
figure,
semilogx(sfdom,sftc([1 2],:)','-o')
xlabel('s freq')
set(gca,'XTick',sort(sfdom))
legend(cleg([1 2]))
...
%% Get sfreq map, averaged over color

sfF0 = zeros(size(imMat{1,1},1),size(imMat{1,1},2),length(sfdom));
for t = 1:length(sfdom)
    for c = 1:length(cdom)
        sfF0(:,:,t) = sfF0(:,:,t)+imMat{t,c}/length(cdom);
    end
end
for t = 1:length(sfdom)
    sfF0(:,:,t) = ifft2(abs(fft2(hh)).*fft2(sfF0(:,:,t))); 
end
...
ma = max(sfF0,[],3);  %Do this first so that it has dF/F units (for later)
mi = min(sfF0,[],3);
%mapmag = (ma-mi)./(ma+mi);
sfmapmag = ma-mi;
sfmapmag = ma;

sfDomTens = zeros(size(sfF0));
for i = 1:length(sfdom)
    sfDomTens(:,:,i) = log2(sfdom(i));
end

[mi dum] = min(sfF0,[],3);
for i = 1:length(sfdom)
    sfF0(:,:,i) = phi(sfF0(:,:,i)-mi);
end

sumMat = sum(sfF0,3);
for i = 1:length(sfdom)
    sfF0(:,:,i) = sfF0(:,:,i)./sumMat;
end

sfMap = sum(sfF0.*sfDomTens,3); %center of mass
sfMap = 2.^sfMap;  %put back in the right units


%sfMap = (sfF0(:,:,1)./sfF0(:,:,4));

%sfMap = (sfF0(:,:,1)-sfF0(:,:,4))./(sfF0(:,:,1)+sfF0(:,:,4));
%sfMap2 = (sfF0(:,:,2)-sfF0(:,:,3))./(sfF0(:,:,2)+sfF0(:,:,3));
b = phi(sfF0(:,:,1));
a = max(sfF0,[],3);
sfMap = (a-b)./(a+b);

% sfMap(find(sfMap<-1)) = -1;
% sfMap(find(sfMap>1)) = 1;
...
for c = 1:length(cdom)
    ...
    ma = max(tempAll{c},[],3);  %Do this first so that it has dF/F units (for later)
    mi = min(tempAll{c},[],3);
    %mapmag = (ma-mi)./(ma+mi);
    sfmapmag = ma-mi;
    sfmapmag = ma;
    
    sfDomTens = zeros(size(tempAll{c}));
    for i = 1:length(sfdom)
        sfDomTens(:,:,i) = log2(sfdom(i));
    end
    
    [mi dum] = min(tempAll{c},[],3);
    for i = 1:length(sfdom)
        tempAll{c}(:,:,i) = phi(tempAll{c}(:,:,i)-mi);
    end
    
    sumMat = sum(tempAll{c},3);
    for i = 1:length(sfdom)
        tempAll{c}(:,:,i) = tempAll{c}(:,:,i)./sumMat;
    end
    
    sfMapAll{c} = sum(tempAll{c}.*sfDomTens,3); %center of mass
    sfMapAll{c} = 2.^sfMapAll{c};  %put back in the right units
...
figure
ax1 = subplot(1,4,1);
plotColorImage(colorMap,imstate,aBound,ax1)
ax2 = subplot(1,4,2);
plotSFImage(sfMap,sfdom,imstate,aBound,ax2)
ax3 = subplot(1,4,3);
plotHorRetImage(imstate,aBound,Emax,ax3)
ax4 = subplot(1,4,4);
plotVertRetImage(imstate,aBound,Emax,ax4)
...
%     ax(c) = subplot(2,4,c);
    ax(c) = subplot(2,2,c);
    plotSFImage(sfMapAll{c},sfdom,imstate,aBound,ax(c))
    title(['SF Bandpass factor; ' cleg{c}])
    
    subplot(2,2,c+2)
    scatter(imstate.fmaps{2}(id),sfMapAll{c}(id),'.k')
    xlabel('horizontal ret'), ylabel('Bandpass factor')
end
...
areaScatter(colorMap,colorF0,sfMap,cdom,sfdom,imstate,aBound,areaOrder,areaNames,Emax)

%% Plot images

function plotColorImage(colorMap,imstate,aBound,ax)
...
function plotSFImage(sfMap,sfdom,imstate,aBound,ax)
...
plotRange = log2([sfdom(1) sfdom(end)]); %octaves
map = log2(sfMap);

plotRange = [0 1];
map = sfMap;
imagesc(xdom,ydom,map,'AlphaData',aBound,plotRange), title('spatial frequency'), axis image, colorbar
% plotExLocs((E_yx-1)/pixpermm)
hold on
...
colorbar(ax,'YTick',iddom,'YTickLabel',domcell)
...
function plotHorRetImage(imstate,aBound,Emax,ax)
...
function plotVertRetImage(imstate,aBound,Emax,ax)
...
function areaScatter(colorMap,colorF0,sfMap,cdom,sfdom,imstate,aBound,areaOrder,areaNames,Emax)
...
mapplot = colorF0(:,:,2)./(colorF0(:,:,1)+colorF0(:,:,2));
...
%cid = 2:7;
%gdomH = cdom(cid);
%y = zeros(length(sfdom)*length(gdomH),size(imMatF{1},1), size(imMatF{1},2));
...
for i = 1:size(E_yx,1)
    ...
    kern = reshape(kern,length(sfdom),length(cdom));
    kernfit = yhat(:,E_vecloc);
    kernfit = reshape(kernfit,length(sfdom),length(cdom));
    ma = max([kern(:); kernfit(:)]);
    mi = min([kern(:); kernfit(:)]);
...
%         for k = 1:length(sfdom) %Loop each t_period
...
%     for i = 2:length(areaID) %loop each area
%         
%         subplot(length(sfdom),length(areaID)-1,q)
%         mudum = squeeze(gmean{i-1}(:,k,:))';      
%         stddum = squeeze(gstd{i-1}(:,k,:))'; 
%         %imagesc(cdom,retdom,mudum'), xlabel('theta'), ylabel('retinotopy')
%         errorbar(cdom'*ones(1,size(mudum,2))./sfdom(k),mudum,stddum), xlabel('col/tp')
%         
%         areastr = '';
%         if ~isempty(varargin)
%             areaNames = varargin{1};
...
%         end
%         
%          title([areastr '; tp = ' num2str(sfdom(k))])
%          if q == 1
%              legend(leg)
%          end
...
% figure,
% q = 1;
% [gdomM bdomM] = meshgrid(cdom,sfdom);
% ratDom = gdomM./bdomM;
...
%         hold on
%         %title(['tp = ' num2str(sfdom(k)) '; area' num2str(i-1)])
%         %xlim([cdom(1)-.1 cdom(end)+.1])
%     end
%     q = q+1;
% end
...
%         imagesc(cdom,sfdom,dum), ylabel('t_period'), xlabel('theta')
%          %title(['tp = ' num2str(sfdom(k)) '; area' num2str(i-1)])
%          axis xy
%          
%          
