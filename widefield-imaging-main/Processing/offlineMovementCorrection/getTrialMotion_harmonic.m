function [Pxhat Pyhat] = getTrialMotion_harmonic(CH)
...
for i = 2:id(end)
    ...
    Pxhat = Pxhat + abs(Hxdum)*cos(2*pi*fdom(i)*tdomhat + angle(Hxdum));
    Pyhat = Pyhat + abs(Hydum)*cos(2*pi*fdom(i)*tdomhat + angle(Hydum));
